#!/usr/bin/env python3
"""Update ALL remaining portfolio cards with new button design"""
...
for match in reversed(cards):
    ...
    # Check if it has the old footer structure (with port-card-year and any link)
    if 'port-card-year' in card_section and 'port-case-link' in card_section:
        # More flexible pattern - matches any link text (View Project or View Video)
        old_footer_pattern = r'<div class="port-card-footer">\s*<span class="port-card-year">.*?<\/span>\s*<a href="javascript:void\(0\)" class="port-case-link" onclick="event\.stopPropagation\(\);viewProject\(this\);">View[^<]*&rarr;<\/a>\s*<\/div>'
...
# Count remaining
remaining = len(re.findall(r'port-card-year', content))
print(f"Remaining cards with old footers: {remaining}")
